overallreport: async (req, res) => {
    ...
    const customisedcompenstatedprreport = util.asyncfunctionbuilder(
        helperfunctionsdata.helperfunctions.customisedcompenstateddailyprreport
            .function
    );
    const dailydata = await customisedcompenstatedprreport(
        dbconnection,
        {
            prreport,
        },
        ...
        {
            printerval:
                helperfunctionsdata.helperfunctions
                    .customisedcompenstateddailyprreport.printerval,
    ...
    try {
        blockdata = await blockdevicedata.getblockdatabydevices(
            dbconnection,
            format(startOfDay(new Date(req.query.startdate)), 'yyyyMMddHHmm'),
            format(endOfDay(new Date(req.query.startdate)), 'yyyyMMddHHmm'),
        ...
        Object.keys(mainmeterdevices).forEach((y) => {
            if (mainmeterdevices[y].devicemeta.devicename == 'MM1') {
                ...
                mainmeterdevices[y].minexportvalue = minmainmeterexportvalue.devices[y].parameters.MM1_TOT_ACT_EXP
                    || 0;
                const maxmainmeterexportvalue = lodash.maxBy(
                    Object.values(data),
                    (x) => x.devices[y].parameters.MM1_TOT_ACT_EXP
                );
                mainmeterdevices[y].maxexportvalue = maxmainmeterexportvalue.devices[y].parameters.MM1_TOT_ACT_EXP
                    || 0;
                ...
                mainmeterdevices[y].minimportvalue = minmainmeterimportvalue.devices[y].parameters.MM1_TOT_ACT_IMP
                    || 0;
                const maxmainmeterimportvalue = lodash.maxBy(
                    Object.values(data),
                    (x) => x.devices[y].parameters.MM1_TOT_ACT_IMP
                );
                mainmeterdevices[y].maximportvalue = maxmainmeterimportvalue.devices[y].parameters.MM1_TOT_ACT_IMP
                    || 0;
                ...
                mainmeterdevices[y].minexportvalue = minmainmeterexportvalue.devices[y].parameters.MM2_TOT_ACT_EXP
                    || 0;
                const maxmainmeterexportvalue = lodash.maxBy(
                    Object.values(data),
                    (x) => x.devices[y].parameters.MM2_TOT_ACT_EXP
                );
                mainmeterdevices[y].maxexportvalue = maxmainmeterexportvalue.devices[y].parameters.MM2_TOT_ACT_EXP
                    || 0;
                mainmeterdevices[y].unit = tempexportvalueunit.parameterunit;
                ...
                mainmeterdevices[y].minimportvalue = minmainmeterimportvalue.devices[y].parameters.MM2_TOT_ACT_IMP
                    || 0;
                const maxmainmeterimportvalue = lodash.maxBy(
                    Object.values(data),
                    (x) => x.devices[y].parameters.MM2_TOT_ACT_IMP
                );
                mainmeterdevices[y].maximportvalue = maxmainmeterimportvalue.devices[y].parameters.MM2_TOT_ACT_IMP
                    || 0;
                mainmeterdevices[y].unit = tempimportvalueunit.parameterunit;
            }
        });
        ...
        Object.keys(checkmeterdevices).forEach((y) => {
            if (checkmeterdevices[y].devicemeta.devicename == 'CM1') {
                ...
                checkmeterdevices[y].minexportvalue = mincheckmeterexportvalues.devices[y].parameters.CM1_TOT_ACT_EXP
                    || 0;
                const maxcheckmeterexportvalues = lodash.maxBy(
                    Object.values(data),
                    (x) => x.devices[y].parameters.CM1_TOT_ACT_EXP
                );
                checkmeterdevices[y].maxexportvalue = maxcheckmeterexportvalues.devices[y].parameters.CM1_TOT_ACT_EXP
                    || 0;
                ...
                checkmeterdevices[y].minimportvalue = mincheckmeterimportvalues.devices[y].parameters.CM1_TOT_ACT_IMP
                    || 0;
                const maxcheckmeterimportvalues = lodash.maxBy(
                    Object.values(data),
                    (x) => x.devices[y].parameters.CM1_TOT_ACT_IMP
                );
                checkmeterdevices[y].maximportvalue = maxcheckmeterimportvalues.devices[y].parameters.CM1_TOT_ACT_IMP
                    || 0;
                ...
                checkmeterdevices[y].minexportvalue = mincheckmeterexportvalues.devices[y].parameters.CM2_TOT_ACT_EXP
                    || 0;
                const maxcheckmeterexportvalues = lodash.maxBy(
                    Object.values(data),
                    (x) => x.devices[y].parameters.CM2_TOT_ACT_EXP
                );
                checkmeterdevices[y].maxexportvalue = maxcheckmeterexportvalues.devices[y].parameters.CM2_TOT_ACT_EXP
                    || 0;
                checkmeterdevices[y].unit = tempexportvalueunit.parameterunit;
                ...
                checkmeterdevices[y].minimportvalue = mincheckmeterimportvalues.devices[y].parameters.CM2_TOT_ACT_IMP
                    || 0;
                const maxcheckmeterimportvalues = lodash.maxBy(
                    Object.values(data),
                    (x) => x.devices[y].parameters.CM2_TOT_ACT_IMP
                );
                checkmeterdevices[y].maximportvalue = maxcheckmeterimportvalues.devices[y].parameters.CM2_TOT_ACT_IMP
                    || 0;
                checkmeterdevices[y].unit = tempimportvalueunit.parameterunit;
            }
        });
        ...
        data.forEach((e) => {
            ...
            if (tempvalminwmsghi.length > 0) {
                ...
                tempminwmsmoduletemperature.push(
                    isNaN(lodash.meanBy(tempmoduletemp))
                        ? 0
                        : lodash.meanBy(tempmoduletemp)
                );
            }
            ...
            peekpower.push(
                lodash.sumBy(
                    temppeekpowerdevices,
                    (x) => x.parameters.PQM_ACT_PWR
                )
            );
        });
        const page1data = [];
        ...
        plantstoptime = new Date(data[data.length - 1].localstarttimestamp)
            || new Date();
        ...
        const tempwmsmoduletempdatavalues = lodash.sumBy(wmssurtemp) / wmssurtemp.length;
        ...
        res.setHeader(
            'Content-Disposition',
            `attachment; filename=${`overallreport${format(
                new Date(),
                'yyyyMMddHHmmss'
            )}.xlsx`}`
        );
        ...
        log.error(error);
        return res.status(500).send(util.error(error));
    }
},